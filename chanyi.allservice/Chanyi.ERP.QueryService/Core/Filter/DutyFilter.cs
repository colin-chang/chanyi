...
{
    [DataContract]
    public class DutyFilter : BaseFilter
    {
        [DataMember]
        ...
        protected override GeneratorDelegate CreateGenerator()
        {
            return (out List<MySqlParameter> ps) =>
            {
                ...
                {
                    listWhere.Add("u.Name like @Name");
                    ps.Add(new MySqlParameter("Name", this.Name.Wrap("%")));

                }
                if (!string.IsNullOrWhiteSpace(Desc))
                {
                    listWhere.Add("d.`Desc`=@Desc");
                    ps.Add(new MySqlParameter("Desc", this.Desc));
                }
                if (!string.IsNullOrWhiteSpace(DepartmentName))
                {
                    listWhere.Add("(DepartmentName like @DepartmentName)");
                    ps.Add(new MySqlParameter("DepartmentName", this.DepartmentName.Wrap("%")));
                }
                if (IsEnabled != null)
                {
