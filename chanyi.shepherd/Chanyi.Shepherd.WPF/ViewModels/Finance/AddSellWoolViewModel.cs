...
{
    class AddSellWoolViewModel : AddViewModel
    {
        public AddSellWoolViewModel(UIElement error)
        {
            this.errorControl = error;
            this.InitializeBindData();
        }

        public AddSellWoolViewModel(bool withinInitilization) { }
        ...
        [EntityProperty]
        [FloatNumber(0, int.MaxValue, ErrorMessage = "出售数量输入不合法")]
        [Required(ErrorMessage = "出售数量必填必填")]
        ...
        public DelegateCommand<UIElement> SubmitCommand
        {
            get
            {
                ...
                {
                    var result = this.Service.AddSellWool(float.Parse(this.Amount), decimal.Parse(this.Price), this.PurchaserId, (DateTime)this.OperationDate, this.PrincipalId, this.UserId, this.Remark);

                    this.errorControl = err;
                    if (!ValidateFailedServiceResult<string>(result))
                        return;
                    if (this.Continue2Add("羊毛出售记录添加成功"))
                        return;
